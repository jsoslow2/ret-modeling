...
changing_new_users <- function(ret_list, new_user_base, change_in_new_users) {
...
curve_fit <- function(d1, d7, d30, len) {
  ...
  ret = pmax(survivalBG(alpha, beta, len) - gamma, 0)
...
optimization <- function(d1, d7, d30, new_users, change_in_new_users = 0) {
  ret = curve_fit(d1, d7, d30, 1:365)
  
  if (change_in_new_users != 0) {
    df = changing_new_users(ret, new_users, change_in_new_users)
  } else {
...
server <- function(input, output, session) {
  ...
  observeEvent(input$generate, {
    df = optimization(input$d1, input$d7, input$d30, input$new_users, input$new_user_change)
